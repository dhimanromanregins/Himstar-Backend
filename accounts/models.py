...
class Register(models.Model):
    ...
    cover_image = models.ImageField(storage=AzureMediaStorage(), upload_to='cover-images/', blank=True, null=True)
    cover_image_url = models.CharField(max_length=1000, blank=True, null=True)
    profile_image = models.ImageField(storage=AzureMediaStorage(), upload_to='profile-images/', blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        ...
        update_fields = []
        if self.cover_image:
            self.cover_image_url = f"{settings.AZURE_FRONT_DOOR_DOMAIN.rstrip('/')}/{self.cover_image.name.lstrip('/')}"
            update_fields.append("cover_image_url")

        if self.profile_image:
            self.profile_image_url = f"{settings.AZURE_FRONT_DOOR_DOMAIN.rstrip('/')}/{self.profile_image.name.lstrip('/')}"
